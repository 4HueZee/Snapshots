...
def map_universal_category(raw_name):
    if not raw_name: return None
    u = raw_name.upper().strip()
    if "HQ" in u or "CHARACTER" in u or "HERO" in u or "LEADER" in u or "WARLORD" in u or "COMMAND" in u:
        return "CHARACTERS"
    if "TROOP" in u or "BATTLELINE" in u or "CORE" in u:
        return "BATTLELINE"
    if "TRANSPORT" in u:
...
def create_db(db_path):
    ...
    # 5. Game System Metadata Table
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS game_system_metadata (
...
def parse_file(conn, file_path, repo_name, category_map):
    ...
    f_id = normalize_id(root.get('id')) or os.path.basename(file_path).replace('.cat', '').replace('.gst', '')
    f_name = root.get('name') or f_id
    ...
    def process_node(node, parent_id, current_profile_name=None, current_profile_type=None):
        ...
        if node_id:
            category = map_universal_category(node_name) or map_universal_category(entry_type)

            for child in node:
                child_tag = child.tag.split('}')[-1]
                if child_tag == 'categoryLink':
                    cat_name = child.get('name') or category_map.get(child.get('targetId'))
                    is_primary = child.get('primary') == 'true'
                    if cat_name:
                        cat_mapped = map_universal_category(cat_name)
                        if cat_mapped and (is_primary or not category):
                            category = cat_mapped

            cursor.execute('''
                INSERT OR REPLACE INTO singularities
                (id, gamesystem_id, faction_id, name, xml_tag, entry_type, value, parent_id, target_id, link_type, category, gamesystem_name, faction_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (node_id, gs_id, f_id, node_name or node_tag, node_tag, entry_type, node_value, parent_id, target_id, entry_type, category, gs_name, f_name))
...
def run_shaper(source_dir, output_db, repo_id):
    ...
    if integrity and integrity[0] == "ok":
        print(f"  [INTEGRITY OK] SQLite database passed integrity check.")
    else:
        print(f"  [ERROR] Database integrity check failed: {integrity}")
        conn.close()
        if os.path.exists(output_db): os.remove(output_db)
        raise RuntimeError("Shaping failed due to corrupt SQLite database.")
